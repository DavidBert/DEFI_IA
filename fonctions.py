...
def X_forecast_jour(ds,coordonnees_stations):
...
def X_forecast(journées,numéros_stations,data_path):
    ...
    for i in range(len(journées)):
        ...
        ################
        if os.path.exists(data_path+"/2D_arome/2D_arome_"+fichier+".nc") == False:
            print('Le fichier n\'existe pas, un fichier générique X_forecast va être créer avec des -pi')
            X_forecast = np.concatenate((X_forecast, np.ones((1,len(numéros_stations),24,7))*(-np.pi)), axis=0)
        else:
            ds = xr.load_dataset(data_path+"/2D_arome/2D_arome_"+fichier+".nc", engine="netcdf4")
            if len(ds.data_vars) != 9:
                ...
                if valid_shape != [7,227,315,25]:
                    ...
                    X_forecast_day = X_forecast_jour(ds,coordonnees_stations).reshape((1,len(numéros_stations),24,7))
...
def X_station_train(fichier_csv, journées,coordonnees_stations,numéros_stations,altitude_stations):
...
def X_station_test(fichier_csv,coordonnees_stations,numéros_stations,altitude_stations):
  X_station = []
  for row in csv.reader(open(fichier_csv)):
    X_station.append(row)
  X_station = np.asarray(X_station)
...
def Y_station(fichier_csv, journées,numéros_stations):
...
def entrainement_NN(NN, optimizer, loss, inputs, outputs, epochs, batch_size,folder,name_model):
    ...
    NN.save_weights(folder+'/'+name_model+'.h5')
...
def mise_en_forme_prediction_kagglev2(Y_forecast_model,folder,format_sub,id_test_str):
  ...
  submission_github = np.concatenate((np.reshape(format_sub, (-1,1)), predictions), axis=1)

  pd.DataFrame(submission_github.reshape((len(submission_github),-1))).to_csv(folder+'/predictions_ENM_Les_Rainettes.csv',index=False,header = ['Id','Prediction'])
